...
Snake.prototype.getNextPos = function() {
    ...
    this.pos.forEach(function(value) {
        if (value[0] == nextPos[0] && value[1] == nextPos[1]) {
            //如果数组中的两个数据相等，说明下一个点在蛇身体里能找到，代表撞到自己
            selfcollied = true;
        }
    });
    if (selfcollied) {
...
Snake.prototype.strategies = {
    move: function(format) { //这个参数决定要不要删除最后一个方块  (蛇尾),当传了这个参数表示吃
        //在旧蛇头位置创建新身体 
...
function createFood() {
    ...
    var include = true; //循环跳出的条件，true表示食物的坐标在蛇身上(需要继续循环)，false表示食物不足蛇身上(不循环了)
    while (include) {
        x = Math.round(Math.random() * (td - 1));
        y = Math.round(Math.random() * (tr - 1));
        snake.pos.forEach(function(value) {
            if (x != value[0] && y != value[1]) { //食物坐标不在蛇身上
                include = false;
            }
        });
    ...
    food.pos = [x, y]; //存储生成食物的坐标，用于跟蛇头要走的下一个点对比
...
Game.prototype.over = function() {
        ...
        snake = new Snake();
        game = new Game();
        var starBtnWrap = document.querySelector('.startBtn');
